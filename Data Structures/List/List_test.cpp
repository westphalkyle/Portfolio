...
int main()
{
    ...
    assert(*++it2 == 38);
    assert(*++it2 == 44);
    if(DEBUG) cout << "Pass 6" << endl;
   
    l.pop_back();             //list = 22, 33, 38
    List<int> copy;
    copy = l;      //copy = 22, 33, 38
    if(DEBUG) cout << "Pass 6.1" << endl;
    assert(copy.size() == 0);
    assert(copy.size() == 3);
    List<int>::iterator it3 = copy.begin();
    assert(*it3 == 22);
    assert(*++it3 == 33);
    if(DEBUG) cout << "Pass 7" << endl;
